...
class RuleClassifier:
    ...
    CLAIM_KEYWORDS = [
        ...
        'réclamation', 'plainte', 'réclame', 'je réclame',
        # Problèmes et pannes
        'problème', 'souci', 'bug', 'erreur', 'dysfonctionnement', 'défaillance',
        'panne', 'coupure', 'interruption', 'déconnexion', 'perte de connexion',
        # Non fonctionnement
        'ne fonctionne pas', 'ne marche pas', 'ça ne marche pas', 'ne fonctionne plus',
        'marche plus', 'fonctionne plus', 'impossible de', 'n\'arrive pas', 'ne peut pas',
        # Perte de service - CORRECTION ISSUE-001
        'plus de connexion', 'plus d\'internet', 'plus de réseau',
        'sans connexion', 'sans internet', 'sans réseau',
        'aucune connexion', 'aucun internet', 'aucun réseau',
        # Insatisfaction et mécontentement
        'déçu', 'mécontent', 'insatisfait', 'catastrophe', 'scandale', 'honteux',
        'inadmissible', 'inacceptable', 'ras le bol', 'en ai marre', 'j\'en ai marre',
        # Demandes de remboursement/résiliation
        'remboursement', 'dédommagement', 'compensation', 'résiliation', 'résilier',
        'changer d\'opérateur', 'partir de', 'quitter free',
        # Qualificatifs négatifs forts
        'nul', 'pourri', 'minable', 'catastrophique', 'lamentable', 'pire',
        # Durée prolongée (indicateur de réclamation)
        'depuis plusieurs jours', 'depuis une semaine', 'toujours pas', 'encore rien'
    ]
    
    URGENCE_HAUTE_KEYWORDS = [
        # Urgence explicite
        'urgent', 'urgence', 'immédiat', 'tout de suite', 'rapidement', 'au plus vite',
        # Gravité
        'critique', 'grave', 'sérieux', 'important', 'prioritaire',
        # Impact fort - AMÉLI ORATIONS ISSUE-001 & ISSUE-002
        'panne totale', 'coupure totale', 'coupure complète', 'coupure générale',
        'plus de connexion', 'plus d\'internet', 'plus de réseau',
        'sans connexion', 'sans internet', 'sans réseau',
        'aucune connexion', 'aucun réseau', 'aucun internet',
        'plus rien', 'complètement HS', 'totalement HS',
        # Durée prolongée - CORRECTION ISSUE-001
        'depuis plusieurs jours', 'depuis une semaine', 'depuis des jours',
        r'depuis \d+ jours', r'depuis \d+ semaines', r'depuis \d+ heures',
        r'ça fait \d+ jours', r'ça fait \d+ semaines',
        # Contexte professionnel - CORRECTION ISSUE-001 & ISSUE-002
        'télétravail', 'travail', 'professionnel', 'professionnelle',
        'entreprise', 'société', 'business', 'bureau',
        'en télétravail', 'pour le travail', 'au travail',
        # Réseau entreprise - CORRECTION ISSUE-002
        'réseau entreprise', 'réseau professionnel', 'connexion entreprise',
        # Récurrent
        'toujours', 'encore', 'systématiquement', 'à chaque fois',
        'tous les jours', 'chaque jour', 'en permanence'
    ]
    
    URGENCE_MOYENNE_KEYWORDS = [
        'problème', 'souci', 'bug', 'lenteur', 'ralentissement',
        'parfois', 'de temps en temps', 'occasionnellement'
    ]
    
    # Topics (fibre, mobile, facture)
    FIBRE_KEYWORDS = [
        'fibre', 'box', 'freebox', 'internet', 'wifi', 'connexion internet',
        'débit', 'ligne', 'adsl', 'réseau fixe'
    ]
    
    MOBILE_KEYWORDS = [
        'mobile', '4g', '5g', 'forfait mobile', 'réseau mobile', 'appel',
        'sms', 'data', 'roaming', 'carte sim', 'free mobile'
    ]
    
    FACTURE_KEYWORDS = [
        'facture', 'facturation', 'paiement', 'prélèvement', 'montant',
        'prix', 'tarif', 'abonnement', 'coût', 'euros', '€'
    ]
    ...
    def __init__(self):
        ...
        self.claim_pattern = re.compile(
            '|'.join([re.escape(kw) for kw in self.CLAIM_KEYWORDS]),
            re.IGNORECASE
        )
        
        self.urgence_haute_pattern = re.compile(
            '|'.join([re.escape(kw) if not '\\d' in kw else kw for kw in self.URGENCE_HAUTE_KEYWORDS]),
            re.IGNORECASE
        )
...
if __name__ == '__main__':
    ...
    print(f"\n Statistiques:")
    print(f"   Réclamations: {stats['claims_count']}/{stats['total']} ({stats['claims_percentage']:.1f}%)")
