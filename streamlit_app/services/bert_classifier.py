...
class BERTClassifier:
        ...
        if gpu_available:
            try:
                ...
                if compute_cap in supported_caps:
                    ...
                    logger.warning(f"️  GPU {torch.cuda.get_device_name(0)} ({compute_cap_str}) non compatible - CPU utilisé")
                    logger.warning(f"   PyTorch 2.5.1 supporte: sm_50-90, votre GPU: {compute_cap_str}")
    ...
    def predict_sentiment_batch(self, 
                                texts: List[str], 
